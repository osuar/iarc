...
class angular_pos{
	public:
		...
		char updateFlag;
	private:
		void angular_posCallback(const rxtxserver::AngularPos::ConstPtr& angularposdata);
		ros::NodeHandle n;
		ros::Subscriber angular_pos_sub;
};
angular_pos::angular_pos(){
	angular_pos_sub = n.subscribe<rxtxserver::AngularPos>("angular_pos", 1, &angular_pos::angular_posCallback, this);
}

void angular_pos::angular_posCallback(const rxtxserver::AngularPos::ConstPtr& angularposdata){
	updateFlag = 1;
	x = angularposdata->x;
	y = angularposdata->y;
}
