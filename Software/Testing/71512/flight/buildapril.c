...
/*Start memory location for Accel and Gyro reads, should be moved
	...
	CCPWrite( &WDT.CTRL, 0b00010011);

	/*Start of flight control state machine loop*/
	while(1){
		/*Reset Watcht Dog Timer*/
		asm("wdr");
...
*/
				xbeebuffer[5] = accelcache[0] >> 8;
				xbeebuffer[6] = accelcache[0] & 0xff;
				xbeebuffer[7] = accelcache[1] >> 8;
				xbeebuffer[8] = accelcache[1] & 0xff;
