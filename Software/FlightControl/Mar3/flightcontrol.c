	...
	while(1)
	{
		...
		if(1 == dataflag)
		{
			...
			for(datatransfer = 0; datatransfer <= 4; datatransfer += 2)
			{
				...
				{
					PORTD.OUT ^= 0x80;
					accelreadbuffer[datatransfer / 2] -= ((~imu.readData[datatransfer] + 1) & ~(0x07));
					accelreadbuffer[datatransfer / 2] -= (256 * (~imu.readData[datatransfer + 1] + 1));
				}
				else
				{
					accelreadbuffer[datatransfer / 2] += (imu.readData[datatransfer] & ~(0x07));
			...
			if(320 <= imuread)
			{
				...
				{
					accelreadbuffer[datatransfer] = (accelreadbuffer[datatransfer] / 320);
				}
				newdata = 1;
				imuread = 0;
			}
